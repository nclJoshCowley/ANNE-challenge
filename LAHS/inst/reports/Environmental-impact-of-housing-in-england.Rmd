...
```{r calc-epc-diff}
EPC_diff <- as.numeric(LAHS::EHS$EPceeb12e) - as.numeric(LAHS::EHS$EPceib12e)

# How many are equal
EPC_diff_0 <- mean(EPC_diff == 0)
show_EPC_diff_0 <- LAHS::display_percent(EPC_diff_0, digits = 3)

# How many are +/- 1
EPC_diff_1 <- mean(EPC_diff >= -1 & EPC_diff <= 1)
show_EPC_diff_1 <- LAHS::display_percent(EPC_diff_1, digits = 3)
```
...
different in `r show_EPC_diff_0` of cases.
...
`r LAHS::display_percent(mean(LAHS::EHS$EPceeb12e %in% c("C", "D")), digits = 3)`
...
such as boiler type; we denote these predictors by a $p$ length vector
$\bm{x}_i = (1, x_{i1}, \dots, x_{ip})$ where the 1 is added to all data to fit
an intercept (population average). We can now model the response by
\begin{equation*}
  y_i
