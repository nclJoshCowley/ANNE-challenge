...
```{r fig1_3, fig.cap = "Little to no change in heating systems"}
...
\clearpage <!-- Force Table 1.2 to appear in this subsubsection -->
```{r tbl1_2}
...
\citet{britishgas2022} claim that these are generally the most efficient boilers
"on the market" and that "nearly all modern boilers are A-rated condensing 
boilers" which is a good sign of newer technology that is better for the 
environment is being implemented across all regions.
...
```{r fig1_4, fig.cap = "Newer boiler types being adopted, nationwide"}
...
TODO: Mention Stan MCMC sample length and any problems that might indicate non-convergence.

```{r load-fit-mlm, include = FALSE}
# Fit and cached in own script to be used here (see `LAHS/R/scripts`).
fit_mlm <- readRDS("C:/Users/Joshc/Git/ANNE-challenge/LAHS/cache/mlm_regional_epc.rds")
```

\subsubsection{Population Level Parameters}

TODO: Discuss results once **full** model has been fit in Stan.

```{r fig3_1, fig.asp = 0.9 * knitr::opts_chunk$get("fig.asp"), fig.cap = "Population estimates imply increasing efficiency"}
LAHS::fig3_1_population_effects(fit_mlm)
```

\subsubsection{Region-Level Parameters}

TODO: Discuss results once **full** model has been fit in Stan.

```{r fig3_2, fig.asp = 1.1 * knitr::opts_chunk$get("fig.asp"), fig.cap = "Region-level estimates not significant"}
